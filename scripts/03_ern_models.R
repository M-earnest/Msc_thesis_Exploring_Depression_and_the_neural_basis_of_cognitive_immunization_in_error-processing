...
means <- emmeans(mod_ern_0, ~ soc_condition)
# teste die gegeneinander (i.e., contraste)
contrast(means, 'tukey', adjust = 'fdr')

means <- emmeans(mod_ern_0_int, ~ soc_condition | group )
# teste die gegeneinander (i.e., contraste)
contrast(means, 'tukey', adjust = 'fdr')
...
mod_ern_1 <- lmer(data = filter(data_ern, channel == 'FCz'),
                amplitude ~ group * soc_condition * bdi_z +
                  (1|subject),
                  contrasts = list(group = 'contr.sum',
                                    soc_condition = 'contr.sum'))
#summary(mod_ern_1)
...
mod_ern_2 <- lmer(data = filter(data_ern, channel == 'FCz'),
                amplitude ~ group * soc_condition * des_z +
                  (1|subject),
                  contrasts = list(group = 'contr.sum',
                                    soc_condition = 'contr.sum'))
summary(mod_ern_2)
...
mod_ern_3 <- lmer(data = filter(data_ern, channel == 'FCz'),
                amplitude ~ group * soc_condition * bdi_z + des_z +
                  (1|subject),
                  contrasts = list(group = 'contr.sum',
                                    soc_condition = 'contr.sum'))
summary(mod_ern_3)
...
bdi_mean = mean(data_ern$bdi_z)
bdi_sd = sd(data_ern$bdi_z)

means <- emmeans(mod_ern_3, ~ soc_condition)
# teste die gegeneinander (i.e., contraste)
contrast(means, 'tukey', adjust = 'fdr')

means <- emmeans(mod_ern_3, ~ bdi_z| group, at=list(bdi_z=c(bdi_mean-(bdi_sd), bdi_mean+(bdi_sd))))
# teste die gegeneinander (i.e., contraste)
contrast(means, 'tukey', adjust = 'fdr')

means <- emmeans(mod_ern_3, ~ group | bdi_z, at=list(bdi_z=c(bdi_mean-(bdi_sd), bdi_mean+(bdi_sd))))
# teste die gegeneinander (i.e., contraste)
contrast(means, 'tukey', adjust = 'fdr')

means <- emmeans(mod_ern_3, ~ bdi_z | soc_condition, at=list(bdi_z=c(bdi_mean-(bdi_sd), bdi_mean+(bdi_sd))))
# teste die gegeneinander (i.e., contraste)
contrast(means, 'tukey', adjust = 'fdr')

means <- emmeans(mod_ern_3, ~ group | soc_condition |bdi_z, at=list(bdi_z=c(bdi_mean-(bdi_sd), bdi_mean+(bdi_sd))))
# teste die gegeneinander (i.e., contraste)
contrast(means, 'tukey', adjust = 'fdr')

means <- emmeans(mod_ern_3, ~ soc_condition | bdi_z | group, at=list(bdi_z=c(bdi_mean-(bdi_sd), bdi_mean+(bdi_sd))))
# teste die gegeneinander (i.e., contraste)
contrast(means, 'tukey', adjust = 'fdr')

mod_ern_5 <- lmer(data = filter(data_ern, channel == 'FCz'),
                amplitude ~ group * soc_condition * des_z + bdi_z +
                  (1|subject),
                  contrasts = list(group = 'contr.sum',
                                    soc_condition = 'contr.sum'))
#summary(mod_ern_5)
...
mod_ern_6 <- lmer(data = filter(data_ern, channel == 'FCz'),
                amplitude ~ group * bdi_z * des_z + soc_condition  +
                  (1|subject),
                  contrasts = list(group = 'contr.sum',
                                    soc_condition = 'contr.sum'))
#summary(mod_ern_6)
...
means <- emmeans(mod_ern_6, ~ soc_condition)
contrast(means, 'tukey', adjust = 'fdr')

#mod_ern_7 <- lmer(data = filter(data_ern, channel == 'FCz'),
#                amplitude ~ soc_condition * bdi_z * des_z + group  +
#                  (1|subject),
#                  contrasts = list(group = 'contr.sum',
#                                    soc_condition = 'contr.sum'))
#summary(mod_ern_7)
...
mod_ern_9 <- lmer(data = filter(data_ern, channel == 'FCz'),
                amplitude ~ soc_condition * des_z  + bdi_z + group +
                  (1|subject),
                  contrasts = list(soc_condition = 'contr.sum'))
#summary(mod_ern_9)
anova(mod_ern_9)
...
mod_ern_10 <- lmer(data = filter(data_ern, channel == 'FCz'),
                amplitude ~ soc_condition * des_z * bdi_z + group +
                  (1|subject),
                  contrasts = list(soc_condition = 'contr.sum'))
#summary(mod_ern_9)
anova(mod_ern_10)
plot_model(mod_ern_10, 'int', , title = 'model 9: amplitude ~ acc * soc_condition + (1|subject)',
           base_size = 11,
           color= c('darkgoldenrod', 'navy', 'thistle4'))
...
compare_performance(mod_ern_0, mod_ern_0_int, mod_ern_1, mod_ern_2,
                    ...
                    mod_ern_9, mod_ern_10,  rank = T)
plot(compare_performance(mod_ern_0, mod_ern_0_int, mod_ern_1, mod_ern_2,
                    mod_ern_3, mod_ern_5, mod_ern_6, mod_ern_8,
                    mod_ern_9, mod_ern_10, rank = TRUE))

#### Contrasts model 3: highest performance score + hypothesis
# amplitude ~ group * soc_condition * bdi_z + des_z +
anova(mod_ern_3)

bdi_mean = mean(data_ern$bdi_sum_score)
bdi_sd = sd(data_ern$bdi_sum_score)
bdi_min = min(data_ern$bdi_sum_score)
bdi_max = max(data_ern$bdi_sum_score)
...
mod_ern_plot <- lmer(data = filter(data_ern, channel == 'FCz'),
